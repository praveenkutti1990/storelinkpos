...
<script type="text/javascript">
    ...
    function addInvoice(){
        ...
        var result = WPOS.sendJsonData("purchases/add", JSON.stringify({ref:ref, channel:"manual", discount:0, custid:$("#ninvcustid").val(), processdt:$("#ninvprocessdt").datepicker("getDate").getTime(), duedt:$("#ninvduedt").datepicker("getDate").getTime(), notes:$('#ninvnotes').val()}));
        if (result!==false){
            ...
            WPOS.transactions.openPurchaseTransactionDialog(ref);
        }
    ...
    function reloadInvoiceData(){
        resetSearchBox();
        var result = WPOS.sendJsonData("purchases/get", JSON.stringify({"stime":stime, "etime":etime}));
        if (result!==false){
    ...
    function doSearch(){
        ...
        var data = {ref: ref};
        WPOS.sendJsonDataAsync("purchases/search", JSON.stringify(data), function(sales){
            var itemarray = [];
            if (sales !== false){
    ...
    function exportCurrentInvoices(){
        ...
        WPOS.initSave("purchases-"+WPOS.util.getDateFromTimestamp(stime)+"-"+WPOS.util.getDateFromTimestamp(etime), csv);
    }

    $(function() {
        // get default data
        var data = WPOS.sendJsonData("multi", JSON.stringify({"suppliers/get":"", "purchases/get":{"stime":stime, "etime":etime}}));
        if (data===false) return;
        WPOS.customers.setCustomers(data['suppliers/get']);
        WPOS.transactions.setTransactions(data['purchases/get']);
        var customers = WPOS.customers.getCustomers();
        ...
        datatable = $('#invoicestable').dataTable({
            ...
            "aoColumns": [
                ...
                { "sType": "html", mData:null, sDefaultContent:'<div class="action-buttons"><a class="green" onclick="WPOS.transactions.openPurchaseTransactionDialog($(this).closest(\'tr\').find(\'.reflabel\').attr(\'title\'));"><i class="icon-pencil bigger-130"></i></a><a class="red" onclick="WPOS.transactions.deleteTransaction($(this).closest(\'tr\').find(\'.reflabel\').attr(\'title\'))"><i class="icon-trash bigger-130"></i></a></div>', "bSortable": false }
            ],
            "columns": [
                {},
                {type: "numeric"},
