...
function setActiveMenuItem(secname) {
  ...
  $(".submenu li").removeClass('active'); // add active to clicked

  var li = $('a[href="#!' + secname + '"]').parent('li');
  $(li).addClass('active'); // set the parent item if its a submenu
...
var WPOS; //On load page, init the timer which check if the there are anchor changes

$(function () {
  // initiate WPOS object
  WPOS = new WPOSAdmin(); // init

  WPOS.isLogged(); // dev/demo quick login
...
function WPOSAdmin() {
  ...
  var currentAnchor = '0';
  var currentsec = '';
  var lastAnchor = null; // Are there anchor changes, if there are, calculate request and send
  ...
  this.checkAnchor = function () {
    ...
    if (currentAnchor != document.location.hash) {
      ...
      if (currentAnchor) {
        var splits = currentAnchor.substring(2).split('&'); //Get the section

        sec = splits[0]; // has the section changed

        if (sec == currentsec && currentAnchor.indexOf('&query') != -1) {// load some subcontent
        } else {
          ...
          currentsec = sec; // set menu items active

          setActiveMenuItem(sec); // close mobile menu
          ...
          WPOS.util.showLoader(); //Creates the  string callback. This converts the url URL/#! &amp;amp;id=2 in URL/?section=main&amp;amp;id=2

          delete splits[0]; //Create the params string

          var params = splits.join('&'); //Send the ajax request
  ...
  function startSessionCheck() {
    ...
    getLoginStatus(function (user) {
      if (user == false) WPOS.sessionExpired();
    });
  }
  ...
  function performLogin() {
    ...
    $(loginbtn).val('Proccessing'); // auth is currently disabled on the php side for ease of testing. This function, however will still run and is currently used to test session handling.
    // get form values
  ...
  this.initAdmin = function () {
    ...
    hidePermSections(); // Load needed config
  ...
  function hidePermSections() {
    ...
    curuser.sections.customers > 0 ? $("#menucustomers").show() : $("#menucustomers").hide(); // hide parent items menu if no submenu items visible
  ...
  this.uploadFile = function (event, callback) {
    ...
    WPOS.util.showLoader();
    var files = event.target.files; // Create a formdata object and add the files
    ...
    $.ajax({
      ...
      error: function error(jqXHR, textStatus, errorThrown) {
        // Handle errors here
        alert("There was an error connecting to the server: \n" + response.statusText); // hide loader
          ...
          case "config":
            if (data.type == "deviceconfig") {
              if (data.data.hasOwnProperty('a')) {
                if (data.data.a == "removed") delete WPOS.devices[data.id];
              } else {
                WPOS.devices[data.data.id] = data.data;
                WPOS.locations[data.data.locationid] = {
  ...
  function formatData(input) {
    ...
    if (_typeof(input) === 'object') input = JSON.stringify(input); // replace " with â€œ

    var regexp = new RegExp(/["]/g);
    var output = input.replace(regexp, '""'); //HTML
