 ...
 */
function WPOSPurchases() {
  var transactions = {};
  var items = null; // functions for opening info dialogs and populating data

  var curref;
  var curid; // TRANSACTION DETAILS DIALOG FUNCTIONS

  this.openTransactionList = function (refs) {
    // check input
    if (!refs) return; // Initiate UI

    if (!uiinit) initUI();
    var translist = $("#translist"),
        transdialog = $("#translistdialog");
    ...
    for (var i in refs) {
      translist.append('<tr><td>' + refs[i] + '</td><td><button class="btn btn-sm btn-primary" onclick="WPOS.purchases.openTransactionDialog(\'' + refs[i] + '\');">Details</button></td></tr>');
    }

    transdialog.dialog('open');
    transdialog.dialog("option", "position", {
      my: "center",
      at: "center",
      of: window
    });
  ...
  this.loadTransactionListDetails = function () {
    ...
    for (var ref in trans) {
      ...
      var tran = trans[ref];
      translist.append('<tr><td>' + ref + '</td>' + '<td>' + WPOS.util.getDateFromTimestamp(tran.processdt) + '</td>' + '<td>' + (WPOS.users.hasOwnProperty(tran.userid) ? WPOS.users[tran.userid].username : tran.userid) + '</td>' + '<td>' + WPOS.util.currencyFormat(tran.total) + '</td>' + '<td><button class="btn btn-sm btn-primary" onclick="WPOS.purchases.openTransactionDialog(\'' + ref + '\');">Details</button></td></tr>');
    }

    $("#translistdetailsbtn").hide();
    $("#translistdialog").dialog("option", "position", {
      my: "center",
      at: "center",
      of: window
    });
  ...
  this.openTransactionDialog = function (ref) {
    // Initiate UI
    if (!uiinit) initUI(); // Check for transaction record
    ...
    curref = ref;
    curid = record.id; // Populate general transaction info
    ...
    populateVoidInfo(record); // Populate customer information
  ...
  this.setTransaction = function (trans) {
    if (trans.hasOwnProperty('ref')) transactions[trans.ref] = trans;
  };

  function loadTransactions(refs) {
    var trans = WPOS.sendJsonData("transactions/get", JSON.stringify({
      refs: refs
    }));
  ...
  function populateItemsTable(items) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (taxStr == "") taxStr = WPOS.util.currencyFormat(0.00); // item mod details
      ...
      $(itemtable).append('<tr><td>' + items[i].qty + '</td><td>' + items[i].name + modStr + '</td><td>' + WPOS.util.currencyFormat(items[i].unit) + '</td><td>' + taxStr + '</td><td>' + WPOS.util.currencyFormat(items[i].price) + '</td>' + '<td><div class="action-buttons itembuttons" style="text-align: right;"><a onclick="WPOS.purchases.openInvoiceItemDialog(' + i + ');" class="green"><i class="icon-pencil bigger-130"></i></a><a onclick="WPOS.purchases.deleteInvoiceItem(' + items[i].id + ')" class="red"><i class="icon-trash bigger-130"></i></a></div></td></tr>');
    }
  }
    ...
    for (var i = 0; i < payments.length; i++) {
      ...
      if (payments[i].hasOwnProperty('paydata')) {
        ...
        if (payments[i].paydata.hasOwnProperty('transRef')) {
          paydetailsbtn = "<button onclick='WPOS.purchases.showPaymentInfo(this);' class='btn btn-xs btn-primary' data-paydata='" + JSON.stringify(payments[i].paydata) + "'>Details</button>";
        } // catch cash-outs
      ...
      $(paytable).append('<tr><td>' + WPOS.util.capFirstLetter(method) + '</td><td>' + WPOS.util.currencyFormat(amount) + '</td><td>' + WPOS.util.getShortDate(payments[i].processdt) + '</td><td>' + paydetailsbtn + '<div class="action-buttons paybuttons" style="text-align: right;"><a onclick="openInvoicePaymentDialog(' + i + ')" class="green"><i class="icon-pencil bigger-130"></i></a><a onclick="WPOS.purchases.deleteInvoicePayment(' + payments[i].id + ')" class="red"><i class="icon-trash bigger-130"></i></a></div></td></tr>');
    }
  }
  ...
  function populateRefundTable(record) {
    ...
    if (record.refunddata !== undefined) {
      ...
      for (var i = 0; i < record.refunddata.length; i++) {
        ...
        $(refundtable).append('<tr><td><span class="label label-warning arrowed">Refund</span></td><td>' + WPOS.util.getDateFromTimestamp(tempdata.processdt) + '</td><td><button class="btn btn-sm btn-primary" onclick="WPOS.purchases.showRefundDialog(' + i + ');">View</button></td><td><button onclick="WPOS.purchases.removeVoid(' + curid + ', ' + tempdata.processdt + ');" class="btn btn-sm btn-danger">X</button></td></tr>');
      }
    }

    if (record.voiddata !== undefined && record.voiddata !== null) {
      $(refundtable).append('<tr>><td><span class="label label-danger arrowed">Void</span></td><td>' + WPOS.util.getDateFromTimestamp(record.voiddata.processdt) + '</td><td><button class="btn btn-sm btn-primary" onclick="WPOS.purchases.showVoidDialog();">View</button></td><td><button onclick="WPOS.purchases.removeVoid(' + curid + ', ' + record.voiddata.processdt + ');" class="btn btn-sm btn-danger">X</button></td></tr>');
    }
  }
  ...
  this.showRefundDialog = function (refundindex) {
    ...
    $("#refamount").text(WPOS.util.currencyFormat(record.amount)); // show payment details button if available
  ...
  function calculateItemTotals() {
    ...
    itemprice = isNaN(itemprice) ? 0 : itemprice; // calculate item tax
  ...
  this.showHistoryDialog = function () {
    // Get the data
    WPOS.util.showLoader();
    var data = WPOS.sendJsonData("invoices/history/get", JSON.stringify({
      id: curid
    }));
  ...
  function populateTemplateSelect(element) {
    ...
    for (var i in templates) {
      if (templates[i].type == "invoice") element.append('<option value="' + i + '">' + templates[i].name + '</option>');
    }
  }
  ...
  this.showEmailDialog = function () {
    ...
    emlcc.val('');
    emlbcc.val('');
    $("#emailmessage").html(''); // get customer
    ...
    if (recipient !== false) {
      var $tag_obj = $('#emailto').data('tag'); //programmatically add default to address
  ...
  this.deleteInvoiceItem = function (id) {
    ...
    if (answer) {
      WPOS.util.showLoader();
      var result = WPOS.sendJsonData("invoices/items/delete", JSON.stringify({
        id: curid,
        itemid: id
      }));
  ...
  this.saveInvoicePayment = function () {
    ...
    WPOS.util.showLoader();
    var action,
        paymentid = $("#transpayid").val();
    var data = {
      id: curid,
      processdt: $("#transpaydt").datepicker("getDate").getTime(),
  ...
  this.deleteInvoicePayment = function (id) {
    ...
    if (answer) {
      WPOS.util.showLoader();
      var result = WPOS.sendJsonData("invoices/payments/delete", JSON.stringify({
        id: curid,
        paymentid: id
      }));
  ...
  this.addVoid = function () {
    ...
    if (answer) {
      ...
      var result = WPOS.sendJsonData("sales/adminvoid", JSON.stringify({
        "id": curid,
        "reason": reason
      }));
  ...
  this.updateNotes = function () {
    ...
    if (answer) {
      ...
      var result = WPOS.sendJsonData("sales/updatenotes", JSON.stringify({
        ref: curref,
        notes: notes
      }));

      if (result !== false) {
        transactions[curref].notes = notes; //this.openTransactionDialog(curref); not nessesary
      } // hide loader
  ...
  this.deleteTransaction = function (ref) {
    ...
    if (answer) {
      ...
      if (WPOS.sendJsonData(a, JSON.stringify({
        "id": transactions[ref].id
      })) !== false) {
  ...
  this.removeVoid = function (id, processdt) {
    ...
    if (answer) {
      // show loader
      WPOS.util.showLoader();
      var result = WPOS.sendJsonData("sales/deletevoid", JSON.stringify({
        "id": id,
        "processdt": processdt
      }));
  ...
  this.searchItems = function (query) {
    ...
    if (query !== '') {
      var upquery = query.toUpperCase(); // search items for the text.
    ...
    $(function () {
      ...
      $("#transitemdialog").removeClass('hide').dialog({
        ...
        buttons: [{
          ...
          click: function click() {
            WPOS.purchases.saveInvoiceItem();
          }
        }, {
      ...
      $("#transpaydialog").removeClass('hide').dialog({
        ...
        buttons: [{
          ...
          "class": "btn btn-success btn-xs",
          click: function click() {
            WPOS.purchases.saveInvoicePayment();
          }
        }, {
          html: "<i class='icon-remove bigger-110'></i>&nbsp; Cancel",
          "class": "btn btn-xs",
      ...
      $("#translistdialog").removeClass('hide').css('padding', '0').dialog({
        ...
        buttons: [{
          ...
          click: function click() {
            WPOS.purchases.loadTransactionListDetails();
          }
        }, {
          html: "<i class='icon-remove bigger-110'></i>&nbsp; Close",
          "class": "btn btn-xs",
      ...
      $("#invprocessdt").datepicker({
        dateFormat: "dd/mm/yy"
      });
      $("#invduedt").datepicker({
        dateFormat: "dd/mm/yy"
      });
      $("#invclosedt").datepicker({
        dateFormat: "dd/mm/yy"
      ...
      for (var i in customers) {
        for (var a in customers[i].contacts) {
          if (customers[i].contacts[a].email.indexOf('@') !== -1) emlarr.push(customers[i].contacts[a].email);
        }

        if (customers[i].email.indexOf('@') !== -1) emlarr.push(customers[i].email);
      }
      ...
      $('.email-input').each(function (index, element) {
        try {
          ...
          console.log(e); //display a textarea for old IE, because it doesn't support this plugin or another one I tried!
      ...
      $(".wysiwyg-toolbar").addClass('wysiwyg-style2'); // item autocomplete
      ...
      $("#stitemsearch").autocomplete({
        source: function source(request, response) {
          response(WPOS.purchases.searchItems(request.term));
        },
        ...
        select: function select(event, ui) {
          ...
          $('#transitemtaxid').val(ui.item.taxid); // lock fields
