...
function WPOS() {
  ...
  this.initApp = function () {
    ...
    if (!checkAppCompatibility()) return false; // check online status to determine start & load procedure.
  ...
  this.checkCacheUpdate = function () {
    ...
    var appCache = window.applicationCache; // check if cache exists, if the app is loaded for the first time, we don't need to wait for an update
    ...
    appCache.addEventListener('progress', function (e) {
      ...
      cacheloaded = isNaN(loaded) ? cacheloaded + 1 : loaded; //console.log(cacheloaded);
  ...
  this.initPlugins = function () {
    // load keypad if set
    setKeypad(true); // load printer plugin

    WPOS.print.loadPrintSettings(); // deploy scan apps

    deployDefaultScanApp(); // init eftpos module if available

    if (WPOS.hasOwnProperty('eftpos')) WPOS.eftpos.initiate();
  };
  ...
  function setKeypad(setcheckbox) {
    if (getLocalConfig().keypad == true) {
      ...
      if (setcheckbox) $("#keypadset").prop("checked", true);
    } else {
      if (setcheckbox) $("#keypadset").prop("checked", false);
  ...
  function deployDefaultScanApp() {
    $.getScript('/assets/js/jquery.scannerdetection.js').done(function () {
      ...
      $(window).scannerDetection({
        onComplete: function onComplete(barcode) {
          // switch to sales tab
  ...
  this.userLogin = function () {
    ...
    var loginbtn = $('#loginbutton'); // disable login button

    $(loginbtn).prop('disabled', true);
    $(loginbtn).val('Proccessing'); // get form values

    var userfield = $("#username");
    var passfield = $("#password");
    var username = userfield.val();
    var password = passfield.val(); // hash password

    password = WPOS.util.SHA256(password); // authenticate
    ...
    authenticate(username, password, function (result) {
      if (result === true) {
        ...
        passfield.val('');
        $("#logindiv").hide();
        $("#loadingdiv").show(); // initiate data download/check
  ...
  this.logout = function () {
    ...
    if (answer) {
      ...
      if (sales > 0) {
        ...
        if (answer) this.backupOfflineSales();
      }
  ...
  function authenticate(user, hashpass, callback) {
    ...
    if (online == true) {
      ...
      if (callback) callback(offlineAuth(user, hashpass));
    }
  }
  ...
  var currentuser; // set current user details
  ...
  this.deviceSetup = function () {
    ...
    var locname = $("#newposlocation").val(); // check input
  ...
  function initSetup() {
    ...
    WPOS.util.showLoader(); // get pos locations and devices and populate select lists

    WPOS.sendJsonDataAsync("multi", JSON.stringify({
      "devices/get": "",
      "locations/get": ""
    }), function (data) {
      ...
      for (var i in devices) {
        if (devices[i].disabled == 0 && devices[i].type != "kitchen_terminal") {
          // do not add disabled devs
      ...
      WPOS.util.hideLoader(); // show the setup dialog
  ...
  function loadOnlineData(step, loginloader) {
    ...
    switch (step) {
      case 1:
        $("#loadingbartxt").text("Loading online resources"); // get device info and settings
      ...
      case 4:
        ...
        fetchSalesTable(function (data) {
          ...
          if (offline_num > 0) {
            $("#backup_btn").show(); // check for offline sales on login
  ...
  this.backupOfflineSales = function () {
    ...
    var blob = new Blob([offline_sales], {
  ...
  function canDoOffline() {
    if (getDeviceUUID() !== null) {
      // can't go offline if device hasn't been setup
  ...
  function switchToOffline() {
    if (canDoOffline() == true) {
      ...
      online = false;
      setStatusBar(3, "WPOS is Offline", "The POS is offine and will store sale data locally until a connection becomes available.", 0); // start online check routine

      checktimer = setInterval(doOnlineCheck, 60000);
      if (WPOS.sales.getOfflineSalesNum() > 0) $(".backup_btn").show();
      return true;
    } else {
  ...
  function fetchConfigTable(callback) {
    ...
    return WPOS.sendJsonDataAsync("config/get", JSON.stringify(data), function (data) {
      if (data) {
        //console.log(data);
        if (data == "removed" || data == "disabled") {
          // return false if dev is disabled
          if (data == "removed") removeDeviceUUID();
  ...
  function updateConfig(key, data) {
    console.log("Processing config (" + key + ") update"); //console.log(data);

    if (key == 'item_categories') return updateCategory(data);

    if (key == "deviceconfig") {
      if (data.id == configtable.deviceid) {
        if (data.hasOwnProperty('a') && (data.a == "removed" || data.a == "disabled")) {
          // device removed
          if (data.a == "removed") removeDeviceUUID();
          logout();
        ...
        if (data.data.hasOwnProperty('a')) {
          if (data.data.a == "removed") delete configtable.devices[data.id];
        } else {
          configtable.devices[data.id] = data;
          configtable.locations[data.locationid] = {
  ...
  function deviceSetup(devid, newdevname, locid, newlocname, callback) {
    ...
    WPOS.sendJsonDataAsync("devices/setup", JSON.stringify(data), function (configobj) {
      ...
      if (callback) callback(configobj !== false);
    });
  }
  /**
  ...
  function fetchSalesTable(callback) {
    return WPOS.sendJsonDataAsync("sales/get", JSON.stringify({
      deviceid: configtable.deviceid
    }), function (data) {
      if (data) {
        salestable = data;
  ...
  function loadItemsTable() {
    ...
    if (data != null) {
      itemtable = JSON.parse(data); // generate the stock index as well.
  ...
  function updateCustTable(data) {
    if (_typeof(data) === 'object') {
      custtable[data.id] = data; // add/update index
  ...
  function socketError() {
    if (WPOS.isOnline()) setStatusBar(5, "Update Feed Offline", "The POS is running in online mode.\nThe feed server is disconnected and this terminal will not receive realtime updates.", 0);
    socketon = false;
    authretry = false;
  }
  ...
  this.util = new WPOSUtil(); //this.purchases = new WPOSPurchases();

  if (typeof WPOSEftpos === 'function') this.eftpos = new WPOSEftpos();
...
$(function () {
  ...
  WPOS = new WPOS(); // initiate startup routine
  ...
  toggleItemBox = function toggleItemBox(show) {
    if (show) {
      iboxopen = true;
      ibox.animate({
        width: "100%"
  ...
  iboxhandle.on('mousedown', function () {
    $(window).on('mousemove touchmove', function () {
      ...
      $(window).on('mousemove touchmove', function (e) {
        // get position
        var parent = $("#iboxhandle").parent().parent(); //alert(parent);

        if (parent.offset() != undefined) {
          var parentOffset = parent.offset().left + parent.width();
          var thisOffset = e.pageX; // get width from the right side of the div.

          var relX = parentOffset - thisOffset; // work out optimal size
  ...
  if (document.location.host == "demo.wallacepos.com" || document.location.host == "alpha.wallacepos.com") {
    ...
    if (document.location.host == "alpha.wallacepos.com") login.append('<button class="btn btn-primary btn-sm" onclick="$(\'#loginmodal\').hide();">Hide Login</button>');
  } // window size


  if (WPOS.getLocalConfig().hasOwnProperty("window_size")) $("#wrapper").css("max-width", WPOS.getLocalConfig()["window_size"]); // set padding for item list
